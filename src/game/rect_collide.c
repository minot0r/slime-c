...
{
    if (rect_collide->transparent)
        return;
    SDL_SetRenderDrawColor(renderer->r_w, rect_collide->color.r, rect_collide->color.g, rect_collide->color.b, rect_collide->color.a);
    SDL_Rect rect = {rect_collide->position.x * renderer->scale,
                     rect_collide->position.y * renderer->scale,
                     rect_collide->width * renderer->scale,
                     rect_collide->height * renderer->scale};
...
collide_dir_e is_colliding_y(rect_collide_t *rect_collide_1, vector2_t pos, int width, int height)
{
    ...
    {
        if (pos.y <= rect_collide_1->position.y)
            return TOP;
        if (pos.y + height >= rect_collide_1->position.y + rect_collide_1->height)
            return BOTTOM;
        return NONE;
    }
    return NONE;
...
collide_dir_e is_colliding_x(rect_collide_t *rect_collide_1, vector2_t pos, int width, int height)
{
    ...
    {
        if (pos.x <= rect_collide_1->position.x)
            return LEFT;
        if (pos.x + width >= rect_collide_1->position.x + rect_collide_1->width)
            return RIGHT;
        return NONE;
    }
    return NONE;
